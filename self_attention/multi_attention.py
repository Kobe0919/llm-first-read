...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Project inputs to Q, K, V: (batch_size, num_tokens, d_out)
        queries = self.W_q(x)
        keys = self.W_k(x)
        values = self.W_v(x)
        ...
        # Calculate causal attention weights
        ...
        context_vec = context_vec.transpose(1, 2)  # (batch_size, num_tokens, heads, d_heads)
